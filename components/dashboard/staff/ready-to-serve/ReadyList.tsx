...
import { Soup, CheckCircle, MapPin, Clock } from "lucide-react";
import { useReadyOrders } from "./useReadyOrders";

export const ReadyList: React.FC = () => {
  const { readyOrders, handleDeliver, getMinutesElapsed, getTableCode } = useReadyOrders();
    ...
    <div className="bg-[#F8FAFC] min-h-screen text-slate-800 p-6 space-y-6">
      ...
      <div className="border-b border-slate-200 pb-5">
        ...
        <p className="text-xs text-slate-500">
          Deliver prepared food items to dining tables instantly.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 flex items-center justify-between shadow-2xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-bold text-slate-550 block uppercase tracking-wide">
              Expedite Counter
            </span>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm min-h-[350px] flex flex-col">
        {readyOrders.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 text-center text-slate-400 py-16">
            ...
            <p className="text-sm font-semibold text-slate-505">All Orders Served</p>
            ...
            {readyOrders.map((order) => (
              ...
              >
                <div>
                  ...
                  <ul className="space-y-2 text-sm text-slate-700">
                    {order.items.map((item, idx) => (
                      <li
                        key={idx}
                        className="flex justify-between items-center bg-white rounded-xl p-2.5 border border-slate-150"
                      >
                        <div>
                ...
                <button
                  ...
                  className="w-full rounded-xl bg-emerald-600 hover:bg-emerald-500 py-3 text-xs font-bold text-white shadow-xs transition flex items-center justify-center gap-2 mt-5 cursor-pointer"
                >
